...
def move(evt):
    global mousepos
    global dx
    newpos = scene1.mouse.project(normal=(0,0,1))
    dx = newpos[0] - mousepos[0]
    print(dx)
...
little_sphere = arrow(frame=earthframe, pos=earth.pos + (0, 0, earth.radius) + (0, 0, earth.radius / 32),
                      material=materials.emissive, color=(1, 0, 1), shaftwidth=0.02, length=0.12)
...
wx.StaticText(p, pos=(RATE_SLIDER_POSX, RATE_SLIDER_POSY - 20), label='Step Per Second')
